...
// 利用 sizeof()，算出總共要多少音符
int length = sizeof(notes) - 1; // exclude trailing null terminator
// 決定一拍多長，這邊一拍 300 ms
int tempo = 300;

// forward declaration of playNote so it can be used before its definition
...
void loop() {
 ...
 // 如果是空白的話，不撥放音樂
  ...
  // 呼叫 palyNote() 這個 function，將音符轉換成訊號讓蜂鳴器發聲
      playNote(18,notes[i], beats[i] * tempo);
    }
